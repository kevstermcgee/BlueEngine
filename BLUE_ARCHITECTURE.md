...
Keep future gameplay components keyed by entity ID rather than names or mesh indices. Rendering objects and collision proxies are deliberately separate: furniture keeps a whole-object semantic envelope, while movement uses separate visible legs, seats and tops so small characters can pass beneath it. The camera has a small near plane and player radius to avoid clipping through walls.
...
Interaction update:
`viewer/interaction.rs` defines Action, Interactions and Feedback. Room entities declare ToggleMonitor, CycleCrystal or Inspect. BE2 no longer binds these legacy actions to player input or displays inspection cards. Left-click starts the seeker wrench swing. E now picks up/drops loose props; right-click and R remain available for future disguise controls. Semantic entities remain available to authoring and future gameplay.
